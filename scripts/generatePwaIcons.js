...
function readSourceBase64(prefix) {
  const files = fs
    .readdirSync(SOURCE_PARTS_DIR)
    .filter((name) => name.startsWith(prefix) && name.includes(".b64.part"))
    .sort()

  if (!files.length) {
    throw new Error(`No icon source parts found for ${prefix}`)
  }
...
function makeMaskable(source, size = 512, innerSize = 400) {
  const output = Buffer.alloc(size * size * 3, 0)
  const scaled = resizeImage(source, innerSize, innerSize)
  const start = Math.floor((size - innerSize) / 2)

  for (let y = 0; y < innerSize; y += 1) {
    for (let x = 0; x < innerSize; x += 1) {
      const sourceIndex = (y * innerSize + x) * 3
      const outputIndex = ((start + y) * size + (start + x)) * 3
      output[outputIndex] = scaled.pixels[sourceIndex]
      output[outputIndex + 1] = scaled.pixels[sourceIndex + 1]
      output[outputIndex + 2] = scaled.pixels[sourceIndex + 2]
    }
  }
