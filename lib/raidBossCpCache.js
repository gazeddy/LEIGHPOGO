...
function normaliseRaidBoss(value, tierKey) {
  ...
  return {
    ...
    // The containing key is authoritative. In particular, Mega entries live
    // under current.mega even when the entry also contains a numeric tier.
...
function bossCategoryFlags(boss) {
  ...
  const form = String(boss.form ?? "").toLowerCase().replace(/[_-]+/g, " ");
  const name = String(boss.name ?? "").toLowerCase();

  return {
    isMega: tier === "mega" || form.includes("mega") || name.startsWith("mega "),
...
function categoryPreferenceScore(item, boss) {
  ...
  if (item.category === "shadow") {
    // PoGoAPI does not expose Shadow as a containing tier, so a name match must
    // remain usable when the record is otherwise a normal five-star entry.
