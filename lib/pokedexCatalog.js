...
function buildPreferredPvpokeRowsByPokemon(rows) {
  const rowsByPokemon = new Map();
  ...
  return new Map(
    [...rowsByPokemon.entries()].map(([pokemonId, pokemonRows]) => [
      pokemonId,
      [...pokemonRows].sort(
        (left, right) =>
          pvpokeFormPreference(left) - pvpokeFormPreference(right)
      )[0],
    ])
  );
}
...
function buildSecondMoveCostByPokemon(rows) {
  ...
  for (const [pokemonId, preferred] of buildPreferredPvpokeRowsByPokemon(
    rows
  )) {
...
function buildPvpokeBuddyDistanceByPokemon(rows) {
  const distancesByPokemon = new Map();

  for (const [pokemonId, preferred] of buildPreferredPvpokeRowsByPokemon(
    rows
  )) {
    const distance = Number(preferred?.buddyDistance);
    if (Number.isFinite(distance) && distance > 0) {
      distancesByPokemon.set(pokemonId, distance);
    }
  }
...
function buildMegaEvolutionsByPokemon(rows) {
  const megaEvolutionsByPokemon = new Map();

  for (const row of Array.isArray(rows) ? rows : []) {
    const pokemonId = Number(row?.pokemon_id);
    const firstTimeEnergy = Number(row?.first_time_mega_energy_required);
    const repeatEnergy = Number(row?.mega_energy_required);

    if (
      !Number.isInteger(pokemonId) ||
      !Number.isFinite(firstTimeEnergy) ||
      firstTimeEnergy <= 0 ||
      !Number.isFinite(repeatEnergy) ||
      repeatEnergy <= 0
    ) {
      continue;
    }

    const form = prettyForm(row.form);
    const megaName =
      optionalString(row.mega_name) ||
      `Mega ${optionalString(row.pokemon_name) || `#${pokemonId}`}${
        form ? ` ${form}` : ""
      }`;
    const types = Array.isArray(row.type)
      ? row.type.filter((type) => typeof type === "string" && type.trim())
      : [];
    const megaEvolution = {
      megaName,
      form,
    ...
    const current = megaEvolutionsByPokemon.get(pokemonId) || [];
    const key = [megaName, form || "normal", firstTimeEnergy, repeatEnergy].join(
      "|"
    );
    ...
    if (
      !current.some(
        (candidate) =>
          [
            candidate.megaName,
            candidate.form || "normal",
            candidate.firstTimeEnergy,
            candidate.repeatEnergy,
          ].join("|") === key
      )
    ) {
      current.push(megaEvolution);
      current.sort((left, right) =>
        left.megaName.localeCompare(right.megaName)
      );
      megaEvolutionsByPokemon.set(pokemonId, current);
    }
  }

  return megaEvolutionsByPokemon;
}
...
function buildPokedexCatalog(
  ...
  const pogoApiBuddyDistanceByPokemon = buildBuddyDistanceByPokemon(
    buddyDistancePayload
  );
  const pvpokeBuddyDistanceByPokemon = buildPvpokeBuddyDistanceByPokemon(
    pvpokePokemonRows
  );
  const secondMoveCostByPokemon = buildSecondMoveCostByPokemon(
    pvpokePokemonRows
  );
  const megaEvolutionsByPokemon = buildMegaEvolutionsByPokemon(
    megaPokemonRows
  );
  const pokemon = Object.fromEntries(
    pokemonList.map(({ dexNumber, name }) => [
      dexNumber,
      {
        ...
        buddyDistance:
          pvpokeBuddyDistanceByPokemon.get(dexNumber) ??
          pogoApiBuddyDistanceByPokemon.get(dexNumber) ??
          null,
        secondMoveCost: secondMoveCostByPokemon.get(dexNumber) ?? null,
        megaEvolutions: megaEvolutionsByPokemon.get(dexNumber) || [],
        previous: [],
        next: [],
      },
    ])
...
module.exports = {
  ...
  buildMegaEvolutionsByPokemon,
  buildPokedexCatalog,
  buildPvpokeBuddyDistanceByPokemon,
