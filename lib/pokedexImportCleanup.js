...
export async function deletePokedexImportCompletely(job, ownerId) {
  ...
  if (job.status === "ACCEPTED" && !rollback) {
    throw new Error(
      `Accepted Pokédex import ${job.id} predates rollback tracking and cannot be safely erased automatically.`,
    )
  }

  // Remove private files first. If filesystem cleanup fails, keep the DB row so
  // Clear Notifications can be retried safely. Missing directories are fine.
